...
class TaskBasedNets(ABC, nn.Module):
    ...
    def predict(self, features: torch.Tensor, task_id: str) -> np.ndarray:
...
class PredictionBasedEvaluator:
    ...
    def eval_all_tasks(self, model: Union[resnet18], data_stream: Scenario):
        task_evaluation = dict()
        for task in tqdm(data_stream.tasks, desc=f'Evaluating tasks...'):
            task_eval_metrics = self.eval_one_task(
                model=model, task=task.test, task_id=task.id, nb_classes=10
            )
    ...
    def eval_one_task(
        self,
        model: Union[ resnet18],
        task: Union[Dataset, ResNet],
    ...
    def before_eval_one_task(self, task: Union[Dataset, ResNet], nb_classes: int = -1) -> None:
        for metric in self.metrics:
...
class RepresentationBasedEvaluator:
    ...
    def record_original_representations(
        self, model: TaskBasedNets, task: Union[Dataset, ResNet], task_id: str = "."
    ) -> None:
        model.to(self.device)
        model.eval()
        eval_loader = torch.utils.data.DataLoader(
