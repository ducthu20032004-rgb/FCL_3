The code repository for "Understanding Representation Drift under Spatial and Temporal Homogeneity in Federated Continual Learning" (GLOW2026,IJCAI 2026) in PyTorch. 
# Understanding Representation Drift under Spatial and Temporal Homogeneity in Federated Continual Learning
Federated Continual Learning (FCL) enables decentralized clients to learn from sequential task streams, but models trained in this setting remain vulnerable to catastrophic forgetting. While recent continual learning studies suggest that forgetting is closely related to representation-level changes, how representations evolve under decentralized optimization, client-specific task orders, and repeated aggregation remains insufficiently understood. To provide a comprehensive perspective at the representation level, we present a representation-level diagnostic framework for FCL. The framework measures temporal representation drift, cross-client representation discrepancy, global and local representation similarity, and probing-based representation forgetting across network layers. We report several findings. First, temporal representation drift increases with network depth, while representation similarity decreases correspondingly. Second, client representations remain highly aligned in lower and intermediate layers but diverge more strongly in the final representation layer. Third, representation drift and probing forgetting are related but not linearly coupled, suggesting that drift alone is insufficient to explain all forgetting behavior. Finally, different FCL methods stabilize different parts of the representation hierarchy. These findings provide a representation-centric perspective on FCL and suggest that future algorithms should explicitly control layer-wise drift, client-level alignment, and representation stability.
